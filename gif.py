...
import url_checker
logger = logging.getLogger("main.gif")
    ...
    def set_stop_time(self, stop_time="00:00:00"):
        if not re.match(r"^(2[0-3]|[01]?[0-9]):([0-5]?[0-9]):([0-5]?[0-9])$", stop_time):
            if not re.match(r"^(2[0-3]|[01]?[0-9]):([0-5]?[0-9]):([0-5]?[0-9])\.([0-9]{0,3})$", stop_time):
                ...
                self._stop_time = datetime.datetime.strptime(
                    stop_time, "%H:%M:%S.%f").time()
        else:
            self._stop_time = datetime.datetime.strptime(
                stop_time, "%H:%M:%S").time()

        if self._stop_time <= self._start_time:
            logger.error("stop time is smaller than start time")
            raise ValueError("stop time is smaller than start time")

        max_stop_time = datetime.time(self._metadata["duration"]//3600,
                                      self._metadata["duration"] % 3600//60,
                                      self._metadata["duration"] % 60)
        self._stop_time = min(self.stop_time, max_stop_time)
